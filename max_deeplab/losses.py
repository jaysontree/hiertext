...
def cdice_similarity(input_mask, target_mask, eps=1e-5):
    ...
    #(B, N, 1, HW) * (B, 1, K, HW) --> (B, N, K, HW) 炸内存 16*384*384*1024*1024*4byte = 9Terabyte
    ...
    """
    tgt_mask = target_mask.permute(0,2,1)
    intersection = torch.matmul(input_mask, tgt_mask)
    cardinalities = torch.sum(input_mask, dim=-1, keepdim=True) + torch.sum(tgt_mask, dim=1, keepdim=True)
...
class PQLoss(nn.Module):
    def __init__(self, alpha=0.75, eps=1e-5):
        ...
        self.negative_xentropy = nn.CrossEntropyLoss(reduction='none')
    ...
    def forward(self, input_mask, input_class, target_mask, target_class, target_sizes):
        ...
        with torch.no_grad():
            class_weight = matched_input_class_prob.gather(-1, matched_target_class[:, None]) #(NP, 1)
        ...
        # originally use (- dice) in UnifiedDetector project
        # 这里是矩阵 * 向量 -> 矩阵， 有问题， 应该是逐元素相乘，维度不变。
        l_pos = torch.sum(class_weight.squeeze() * (- dice) + dice_weight * cross_entropy)

        #eqn 11
        negative_target_class = torch.zeros(
            size=(len(negative_class),), dtype=target_class.dtype, device=target_class.device
        )
        l_neg = torch.sum(self.negative_xentropy(negative_class, negative_target_class))

        #eqn 12
        return (self.alpha * l_pos + (1 - self.alpha) * l_neg) / (b * n), (inp_pos_indices, tgt_pos_indices, neg_indices)
 
        # alternatively "balanced" style loss
        ...
        # loss = self.alpha * l_pos + (1 - self.alpha) * l_neg
...
class InstanceDiscLoss(nn.Module):
    ...
    def forward(self, mask_features, target_mask, target_sizes):
        ...
        b, d, h, w = mask_features.shape[:4]
        target_mask = F.interpolate(target_mask, (h, w), mode='nearest')
